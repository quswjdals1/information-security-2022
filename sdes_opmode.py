...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    ...
    x=0;
    while(x!=textlenth):
        result.append(text[x:x+8]);
        x+=8;



    for i,val in enumerate(result):
        result[i]=sdes(val,key,MODE_ENCRYPT)

    for j in result:
        fullbitarray.extend(j);
    print(fullbitarray)
    return fullbitarray
...
def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    fullbitarray=bitarray()
    result=[];
    textlenth=len(ciphertext);
    x=0;
    while(x!=textlenth):
        result.append(ciphertext[x:x+8]);
        x+=8;


    for i,val in enumerate(result):
        result[i]=sdes(val,key,MODE_DECRYPT)
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    while (x != textlenth):
        xor=text[x:x+8]^iv;
        blockres=sdes(xor,key,MODE_ENCRYPT)
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    while (x != textlenth):
        blockres=sdes(ciphertext[x:x+8],key,MODE_DECRYPT)
        xor=blockres^iv;
        result.append(xor)
        iv = ciphertext[x:x + 8]
        x += 8;
